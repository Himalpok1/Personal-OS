...
const CONFIRM_ERROR_COPY: Record<string, string> = {
  parse_result_not_committable:
    "The parser couldn't classify this, so there's nothing to file yet.",
...
/**
 * Never returns an empty string: before 8.4 the mutation had no error path at
 * all. A `validation_failed` -- the corrected tool call refused by the schema,
 * since 9.6 for a title over ENTITY_TITLE_MAX_CHARS -- takes the field-level
 * line ("title must be at most 512 characters") rather than a fixed sentence,
 * because the field it names is the one the owner can edit right here.
 */
export function confirmErrorMessage(error: unknown): string {
  const code = error instanceof ApiClientError ? error.code : undefined;
  return (
    describeValidationError(error) ??
    (code && CONFIRM_ERROR_COPY[code]) ??
