...
// The reminders read model (Checkpoint 9.4, extracted from routes/reminders.ts
// by Checkpoint 9.7 so the Ask lane's Today context can read it without going
// through HTTP). GET /reminders is now a thin caller; its behaviour is
// byte-identical for the same rows, pinned by routes/reminders.test.ts plus
// read-models/reminders.test.ts.
//
// The primary reminder device schedules one local notification per item
// returned here. Until 9.4 the device paged GET /tasks and scheduled the
// parent's single remind_at, so a recurring task reminded exactly once -- for
// the series anchor -- and never again (the A3 debt). This read model derives
// a reminder PER OCCURRENCE server-side, from the parent's remind_at
// wall-clock time and its day-offset from due_at, resolved in
// recurrence_timezone (packages/core's deriveOccurrenceReminder), so "the
// evening before at 8pm" holds on every instance across DST.
//
...
//       The ROUTE leaves one-offs unbounded into the future (the device must
//       schedule a reminder set months out, and refetches must keep listing
//       it). The Today-context caller passes `oneOffHorizon: true`, which
//       additionally bounds them to [.., now + horizon] -- a 7-day context
//       that listed a reminder in March would be lying about its window.
...
// is returned, so a column added to either query can never leak onto the wire
...
export interface BuildRemindersOptions {
  /** One instant per build (the frozen read-model rule). Defaults to the clock. */
  now?: Date;
  /**
   * Bound ONE-OFF reminders to [now - 1h, now + horizon] as well. The route
   * never sets this (see rule (a) above); the Today-context caller always does.
   */
  oneOffHorizon?: boolean;
}

export async function buildRemindersResponse(
  db: Db,
  query: RemindersQuery,
  options: BuildRemindersOptions = {},
): Promise<RemindersResponse> {
  // One instant per build: both queries and every derived comparison use it.
  ...
  const oneOffRows = await db
    ...
    .where(
      and(
        ...
        ...(options.oneOffHorizon ? [lte(tasks.remindAt, horizonEnd)] : []),
      ),
    );
