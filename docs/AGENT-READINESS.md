...
| Read tool: `search_personal_items` | `POST /agent/tools/search_personal_items` → `searchPersonalItems` | permission `context.read` · sensitivity `titles` · ≤ 10 results per type |
...
| Read tool: `get_item_context` | `POST /agent/tools/get_item_context` → `getItemContext` | **`items.read`** · **`bodies`** — the one body-bearing tool, behind its own permission |
